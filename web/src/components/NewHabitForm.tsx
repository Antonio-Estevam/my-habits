...
export function NewHabitForm(){
    ...
    async function creteNewHabit(event: FormEvent){
        event.preventDefault();
        if(!title || weekDays.length === 0){
            return alert("O formulario foi preenchido incorretamente");
        }
    ...
    return(
        <form onSubmit={creteNewHabit} className="w-full flex flex-col mt-6">
            ...
            {availableWeekDays.map((weekDay, index) => {
                return(
                        ...
                        >
                            ...
                            <samp className=' text-white lending-tight'>
                                {weekDay}
                            </samp>
                        </Checkbox.Root>
                            );
            ...
            <button type="submit" className="mt-6 rounded-lg p4 flex items-center justify-center gap-3 font-semibold bg-green-600 hover:bg-green-500 transition-colors">
